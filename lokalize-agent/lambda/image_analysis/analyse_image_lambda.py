...
def lambda_handler(event, context):
    """
    Lambda function handler for both S3 trigger events and Bedrock Agent calls
    """
    ...
    try:
        # Check if this is a Bedrock Agent call
        if 'actionGroup' in event and 'apiPath' in event:
            return handle_bedrock_agent_call(event, context)
        
        # Otherwise, handle as S3 trigger event (existing functionality)
        return handle_s3_trigger_event(event, context)
...
def handle_bedrock_agent_call(event, context):
    ...
    try:
        ...
        if 'requestBody' in event and 'content' in event['requestBody']:
            ...
            return create_bedrock_error_response(event, "Missing request body or content")
        
        if not bucket_name or not object_key:
            return create_bedrock_error_response(event, "Missing bucket_name or object_key parameters")
        
        # Environment variables
        DYNAMODB_TABLE = os.environ.get('DYNAMODB_TABLE_NAME', 'lokalize-agent-table')
        ...
        item_id = analyzer.store_in_dynamodb(
            DYNAMODB_TABLE, bucket_name, object_key, 
            analysis_results, product_info, s3_metadata
        )
        
        # Prepare response for Bedrock Agent
        result = {
            'image_hash': item_id,
            'analysis_summary': f"Analyzed image with {len(analysis_results['product_labels'])} labels and {len(analysis_results['detected_text'])} text elements",
        ...
        return create_bedrock_success_response(event, result)
        
    except Exception as e:
        logger.error(f"Error in Bedrock Agent call: {str(e)}")
        return create_bedrock_error_response(event, str(e))

def handle_s3_trigger_event(event, context):
    """Handle S3 trigger events (existing functionality)"""
...
def create_bedrock_success_response(event, result):
    """Create a successful Bedrock Agent response"""
    return {
        "messageVersion": "1.0",
        "response": {
            "actionGroup": event.get('actionGroup', 'ImageAnalysis'),
